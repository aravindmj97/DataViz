...
from colorcet import bmw, coolwarm, fire, CET_L18, b_cyclic_rygcbmr_50_90_c64, rainbow, gouldian
...
def generateatile(zoom, x, y):
    ...
    csv = ds.Canvas(plot_width=300, plot_height=200)
    temp_frame = frame.drop(['time', 'depth'], axis=1)
    temp_frame.set_index(['lat', 'lon'], inplace=True)
    temp_frame.dropna(inplace=True)
    dr = temp_frame.to_xarray()
    dr = dr.drop_vars(['X', 'Y'])
    agg = csv.quadmesh(dr, 'lon', 'lat')
    ...
    img = tf.shade(agg, cmap=gouldian, how='log')
...
async def startup_event():
    ...
    if pathlib.Path("data/4.csv").exists():
        print("Loading data from file...")
        #td = pd.read_csv('data/stored.csv', usecols=['X', 'Y'])
        pp = csv.read_csv('data/4.csv')
        td = pp.to_pandas()

        #td=td.set_index(['X', 'Y'])
    else:
        print("There's no data/3.csv file!  Execute ./getdata.sh first!")
        sys.exit(1)
